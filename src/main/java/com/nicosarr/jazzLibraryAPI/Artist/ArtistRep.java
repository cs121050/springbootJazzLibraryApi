...
import jakarta.persistence.PersistenceContext;
    ...
    //TODO //otherfiltered call will writen with Criteria API 
/*
 @Transactional // Initialize the lazy hibernate collection
	public List<Artist> retrieveAll() {
		...
		// Access videoContainsArtists here to initialize them, hibernation Lazy
		// alongside many to many relationship , has bound artists with
		// videocontainsartist
	...
	public int create(Artist artist) {
		String sql = "INSERT INTO Artist (artist_name, artist_surname, instrument_id, artist_rank) VALUES (?, ?, ?, ?)";
		return jdbcTemplate.update(sql, artist.getArtist_name(), artist.getArtist_surname(), artist.getInstrument_id(),
	...
	public int update(Artist artist) {
		String sql = "UPDATE artist SET artist_name = ?, artist_surname = ?, instrument_id = ?, artist_rank = ? WHERE artist_id = ?";
		return jdbcTemplate.update(sql, artist.getArtist_name(), artist.getArtist_surname(), artist.getInstrument_id(),
				artist.getArtist_rank(), artist.getArtist_id());

	}

	@Transactional // Initialize the lazy hibernate collection
	public List<Artist> retrieveArtistByName(String artistName, String artistSurname) {
		String jpql = "SELECT a FROM Artist a WHERE a.artist_name= :artistName and artist_surname= :artistSurname"; // a
																													// is
																													// representation
																													// o
																													// an
	...
	public Artist retrieveArtistByNameAndInstrumentId(String artistName, String artistSurname, int instrumentId) {
		String jpql = "SELECT a FROM Artist a WHERE a.artist_name= :artistName and artist_surname= :artistSurname and instrument_id= :instrumentId"; // a
																																						// is
																																						// representation
																																						// o
																																						// an
	...
	public List<Artist> retrieveByDurationId(int durationId) {
		String jpql = "SELECT a.artist_id, a.artist_name, a.artist_surname, a.instrument_id, a.artist_rank "
				+ "FROM Artist a " + "INNER JOIN VideoContainsArtist vca ON a.artist_id = vca.artist.artist_id " + // vca.artist.artist_id
																													// its
																													// many
																													// to
																													// many
	...
	public List<Artist> retrieveArtistByVideoId(int videoId) {
		String jpql = "SELECT a.artist_id, a.artist_name, a.artist_surname, a.instrument_id, a.artist_rank "
				+ "FROM Artist a " + "INNER JOIN VideoContainsArtist vca ON a.artist_id = vca.artist.artist_id "
...
//   }           
	...
	public List<Artist> retrieveByInstrumentIdAndDurationId(int instrumentId, int durationId) {
		...
		query.setParameter("instrumentId", instrumentId);
		query.setParameter("durationId", durationId);
		// return query.getResultList(); // i should do that instead and over with it...
		// but LAZY hibarnate want me to initialise collection before closing it !! so
		// in feeding the object manualy
		List<Artist> resultList = new ArrayList<>();
		List<Object[]> results = query.getResultList();
		for (Object[] row : results) {
			Artist artist = new Artist();
	...
	// TODO// at the start of the epi, retrieve THE FIRST 8 ROWS OF tablerowcount
	// TABLE, STORE the counts of instruments,artists,types,durations and store them
	// in global variables ,,,, so to use them at the "grouped by functions"
	@Transactional
