...
while (($line = $reader.ReadLine()) -ne $null) {
    ...
    if ($isMaster) {
        ...
        $master_id = $obj.id
        $master_url = "https://api.discogs.com/masters/$master_id"
        $title = $obj.title
        ...
        $notes = $obj.notes
        $genres = ConvertTo-JsonString $obj.genres
        ...
        $companies = $null
        $extra_artists = $null
        $images = ConvertTo-JsonString $obj.images
        ...
        $master_url = if ($master_id) { "https://api.discogs.com/masters/$master_id" } else { $null }
        $title = $obj.title
        $year = $obj.year
        $country = $obj.country
        ...
        $uri = $obj.uri
        $thumb = $obj.thumb
        $companies = ConvertTo-JsonString $obj.companies
    ...
    $genres_esc = Escape-SqlString $genres
    $styles_esc = Escape-SqlString $styles
    $date_added_esc = Escape-SqlString $date_added
    $date_changed_esc = Escape-SqlString $date_changed
    $master_url_esc = Escape-SqlString $master_url
    $uri_esc = Escape-SqlString $uri
    $thumb_esc = Escape-SqlString $thumb
    $companies_esc = Escape-SqlString $companies
    ...
    # Build the INSERT statement for Album as a single line
    $sqlAlbum = "INSERT INTO Album (release_id, title, year, country, released, released_formatted, notes, genres, styles, date_added, date_changed, master_id, master_url, uri, thumb, companies, extra_artists, images, labels, tracklist, videos, rating_count, rating_average) VALUES ("
    $sqlAlbum += "$release_id, "
    $sqlAlbum += "'$title_esc', "
    ...
    $sqlAlbum += "$(if ($genres_esc) { "'$genres_esc'" } else { "NULL" }), "
    $sqlAlbum += "$(if ($styles_esc) { "'$styles_esc'" } else { "NULL" }), "
    $sqlAlbum += "$(if ($date_added_esc) { "'$date_added_esc'" } else { "NULL" }), "
    $sqlAlbum += "$(if ($date_changed_esc) { "'$date_changed_esc'" } else { "NULL" }), "
    $sqlAlbum += "$(if ($master_id -ne $null) { $master_id } else { "NULL" }), "
    $sqlAlbum += "$(if ($master_url_esc) { "'$master_url_esc'" } else { "NULL" }), "
    $sqlAlbum += "$(if ($uri_esc) { "'$uri_esc'" } else { "NULL" }), "
    $sqlAlbum += "$(if ($thumb_esc) { "'$thumb_esc'" } else { "NULL" }), "
    $sqlAlbum += "$(if ($companies_esc) { "'$companies_esc'" } else { "NULL" }), "
